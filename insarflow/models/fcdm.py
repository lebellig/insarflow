...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t):
        ...
        t_freq = t_freq.to(dtype=t.dtype)
...
class FCDM(nn.Module):
    ...
    def forward(self, t, x):
        ...
        # time is scalar then we need to expand it to match the batch size
        if t.ndim == 0:
...
#################################################################################
#          InSARFlow-compatible backbone wrappers (img_size, in_ch, out_ch)       #
#################################################################################

class _FCDMBackbone(nn.Module):
    """Adapts FCDM to the standard InSARFlow backbone interface.

    InSARFlow instantiates backbones as:
        backbone_class(img_size=img_size, in_channels=1, out_channels=1)

    FCDM is fully convolutional, so ``img_size`` is silently ignored.
    ``learn_sigma`` is inferred: True iff out_channels == in_channels * 2.
    Subclasses set ``_hidden_size`` and ``_depth``.
    """

    _hidden_size: int
    _depth: list

    def __init__(self, img_size: int, in_channels: int = 1, out_channels: int = 1, **kwargs):
        super().__init__()
        learn_sigma = out_channels == in_channels * 2
        self.model = FCDM(
...
class FCDMSmall(_FCDMBackbone):
    """FCDM-S  — hidden=128,  depth=[2,4,8,4,2]  (~32 M params at in_ch=1)"""
    _hidden_size = 128
    _depth = [2, 4, 8, 4, 2]


class FCDMBase(_FCDMBackbone):
    """FCDM-B  — hidden=256,  depth=[2,4,8,4,2]  (~126 M params at in_ch=1)"""
    _hidden_size = 256
    _depth = [2, 4, 8, 4, 2]


class FCDMLarge(_FCDMBackbone):
    """FCDM-L  — hidden=512,  depth=[2,4,8,4,2]  (~501 M params at in_ch=1)"""
    _hidden_size = 512
    _depth = [2, 4, 8, 4, 2]


class FCDMXLarge(_FCDMBackbone):
    """FCDM-XL — hidden=512,  depth=[3,6,12,6,3] (~695 M params at in_ch=1)"""
    _hidden_size = 512
    _depth = [3, 6, 12, 6, 3]